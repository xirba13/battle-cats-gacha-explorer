...
def compute_targets(
    ...
    Matching is done godfat-name -> master -> stable uid to decide ownership;
...
def _annotate_units(godfat_names: list[str], master: MasterData) -> list[dict]:
    ...
    for n in godfat_names:
        ...
        out.append({
            ...
            "uid": unit["uid"] if unit else None,
...
def apply_followed_path(
    ...
    for action in solution_dict.get("actions", []):
        for name in action.get("units_pulled", []):
            ...
            if unit:
                pulled_uids.add(unit["uid"])
            else:
                unmatched.add(name)
    db.set_owned_bulk(pulled_uids, owned=True, region=region)
    ...
    newly_uids = sorted(pulled_uids - owned_before)
    units_added = [{"uid": uid, "name": master.by_uid[uid]["name"]}
                   for uid in newly_uids]
    ...
    return {
        "history_id": hid,
        "units_added_count": len(newly_uids),    # NEW units only
        "units_pulled_count": len(pulled_uids),  # distinct units pulled (incl. already owned)
